...
def load_model(
    ...
    """
    Returns the image_loade,r model
    :param device:
    :return:
    """

    checkpoint_url = 'https://mildatasets.s3.amazonaws.com/models/robustnessmadry/resnet50imagenet/imagenet_l2_3_0.pt'
    ...
    for k in state_dict['model']:
        if k.startswith('module.model'):
            knormed = k.replace('module.model.', '')
            base_model_state_dict[knormed] = state_dict['model'][k]
...
def load_image_loader() ->  torch.nn.Module:
    ...
    image_loader = v2.Compose(
        [
            ...
            v2.Resize(224),
            v2.CenterCrop(224),
            v2.ToDtype(ch.float32),
            v2.Normalize(mean=new_mean, std=new_std),
        ]
    )
    return image_loader

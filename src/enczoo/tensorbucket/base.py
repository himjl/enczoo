...
class TensorBucket:
    def __init__(
        ...
        self._key_to_i: Dict[str, int] = {
            key: i for (i, key) in enumerate(self.list_keys())
        }
    ...
    def store_tensors(
        self, key_to_tensor: Dict[str, np.ndarray], overwrite_if_exists: bool = False
    ):
        ...
        with h5py.File(self.filepath, "a") as f:
            ...
            for key, tensor in key_to_tensor.items():
                if tensor.shape != self.shape:
                    raise ValueError(
                        f"tensor shape {tensor.shape} does not match expected shape {self.shape}"
                    )
                ...
                if index is not None:
                    if not overwrite_if_exists:
                        raise ValueError(
                            f'Tensor with key "{key}" already exists. Set overwrite_if_exists=True to overwrite.'
                        )

                if not isinstance(tensor, np.ndarray):
                    raise ValueError(
                        f"tensor must be a numpy.ndarray, not {type(tensor)}"
                    )
                ...
                if index is None:
                    ...
                    assert key_group[index] == key.encode("utf-8"), (
                        f"Key mismatch: {key_group[index]} != {key}? Bug."
                    )
    ...
    def delete_tensors(self, keys: List[str]):
        ...
        raise NotImplementedError(
            "Deleting tensors is not yet implemented. This is a non-trivial operation because it requires shifting all tensors after the deleted tensor to the left. This is not currently supported by h5py."
        )
